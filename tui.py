...
def bios():
    ...
    global term2
    global lab
    term1 = ttk.TTkButton(border=False, text="Boot pyOS3", pos=(0,1), parent=root)
    term1.clicked.connect(lambda : boot())
    term2 = ttk.TTkButton(border=False, text="Boot pyOS3 (Developer Mode)", pos=(0,2), parent=root)
    term2.clicked.connect(lambda : cmd())
    lab = ttk.TTkLabel(parent=root, pos=(0,0), text="Boot Manager")
...
def main(d=False):
    ...
    def textedit(par):
        if g:
            ttk.TTkLog.info(f"name: textedit(), args: {par}")
    ...
    def cred(par=root):
        if g:
            ttk.TTkLog.info(f"name: cred(), args: {par}")
    ...
    def help(par=root):
        if g:
            ttk.TTkLog.info(f"name: help(), args: {par}")
    ...
    def terminal(par=root):
        if g:
            ttk.TTkLog.info(f"name: terminal(), args: {par}")
    ...
    def vm1(par=root):
        if g:
            ttk.TTkLog.info(f"name: vm1(), args: {par}")
    ...
    def apps(par=root):
        if g:
            ttk.TTkLog.info(f"name: apps(), args: {par}")
    ...
    def log(msg, run, par):
        run(par)
        if g:
            ttk.TTkLog.info(f"name: log(), args: {msg}{run}{par}")
        ttk.TTkLog.info(msg)

    def logM(msg, run, par):
        run(par)
        if g:
            ttk.TTkLog.info(f"name: logM(), args: {msg}{run}{par}")
        for e in msg:
            ttk.TTkLog.info(e)
    ...
    def dev():
        ...
        g = not g
        if g:
            ttk.TTkLog.info("Developer console enabled")
        else:
            ttk.TTkLog.info("Developer console disabled")
    ...
    ttk.TTkLabel(parent=root, pos=(m,w), text="DEVELOPER MODE", visible=d)
    dev2 = ttk.TTkButton(border=False, text="DEV", pos=(m-6,w), parent=root, visible=d)
    dev2.clicked.connect(lambda : dev())
bios()
root.mainloop()
